...
optimalCounterPicks :: WinRate -> (Character -> Int -> Int -> Logging (Maybe Character, Double))
optimalCounterPicks winRate = (\c w w' -> evs ! (c, w, w'))
    where
        ev :: Character -> Int -> Int -> Logging (Maybe Character, Double)
        ev _ 0 _ = return (Nothing, 1)
        ev _ _ 0 = return (Nothing, 0)
        ev opp winsReq oppWinsReq = do
                options <- mapM option cast
                let maxValue = maximumBy (comparing snd) options
                log opp winsReq oppWinsReq maxValue
                return maxValue
            where
                option cp = do
                    let rate = winRate cp opp
                    let (winValue, _) = runWriter $ evs ! (cp, oppWinsReq, winsReq - 1)
                    let (loseValue, _) = runWriter $ evs ! (opp, winsReq, oppWinsReq - 1)
                    return $ (Just cp, rate * (1 - snd winValue) + (1 - rate) * (snd loseValue))

        log :: Character -> Int -> Int -> (Maybe Character, Double) -> Logging ()
        log opp winsReq oppWinsReq maxValue = tell
            [ printf "EV%s = (%s, %.2f) = max(\n%s)"
                (show (opp, winsReq, oppWinsReq))
                (show $ fst maxValue)
                (snd maxValue)
                (intercalate ",\n  "
                    [ printf "%.2f * (1 - EV%s) + %.2f * EV%s"
                        rate
...
historicalCounterPick :: Character -> Int -> Int -> Logging (Maybe Character, Double)
historicalCounterPick = optimalCounterPicks historicalWinRate

compositeCounterPick :: Character -> Int -> Int -> Logging (Maybe Character, Double)
compositeCounterPick = optimalCounterPicks compositeWinRate
...
prettyPrintCounterPicks
    :: (Character -> Int -> Int -> Logging (Maybe Character, Double))
    -> Character
    -> String
prettyPrintCounterPicks cp c = printf
    ...
    (show c)
    (intercalate "\n"
        [ printf " * %d-%d -> %s (%d%%)" w w' (show $ fromJust char) (floor $ prob * 100 :: Int)
        | w <- [1..4]
        , w' <- [1..4]
        , let (char, prob) = fst $ runWriter $ cp c w w'
        ])
...
printCounterPicks = do
    putStrLn "Historical counterpicks\n"
    mapM_ (putStrLn . (prettyPrintCounterPicks historicalCounterPick)) cast

    putStrLn "Composite counterpicks\n"
    mapM_ (putStrLn . (prettyPrintCounterPicks compositeCounterPick)) cast
